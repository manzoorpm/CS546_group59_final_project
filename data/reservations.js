...
async function createReservation(userId,restaurantId,reservationTime,reservationDate,people){
    ...
    reservationTime = helper.checkIsProperString(reservationTime,"Reservation Time");

    reservationDate = helper.checkIsProperString(reservationDate,"Reservation Date");
    ...
    if (!insertInfo.insertedId || !insertInfo.acknowledged){
        ...
        const reservation = await reservationCollection.findOne({_id: ObjectId(newId)});
...
async function getAllUserReservations(userId){
    ...
    const reservationCollection = await reservations();
    const reservationList = await reservationCollection.find({userId:ObjectId(userId)}).toArray();
    if(!reservationList){
    ...
    for(let i=0;i<reservationList.length;i++){
        ...
        reservationList[i].restaurantId = reservationList[i].restaurantId.toString();
    }
...
async function getUpcomingUserReservations(userId){
    ...
    const reservationCollection = await reservations();
    const reservationList = await reservationCollection.find({userId:ObjectId(userId),status:"upcoming"}).toArray();
    if(!reservationList){
        throw `No Reservations for the user`
    }
    for(let i=0;i<reservationList.length;i++){
...
async function getAllRestaurantReservations(restaurantId){
    ...
    const reservationCollection = await reservations();
    const reservationList = await reservationCollection.find({restaurantId:ObjectId(restaurantId)}).toArray();
    if(!reservationList){
...
async function getUpcomingRestaurantReservations(restaurantId){
    ...
    const reservationCollection = await reservations();
    const reservationList = await reservationCollection.find({restaurantId:ObjectId(restaurantId),status:"upcoming"}).toArray();
    if(!reservationList){
        throw `No Reservations for the restaurant`
    }
    for(let i=0;i<reservationList.length;i++){
...
async function getReservationById(reservationId){
    ...
    const reservationCollection = await reservations();
    let reservation = await reservationCollection.findOne({_id: ObjectId(reservationId)});
